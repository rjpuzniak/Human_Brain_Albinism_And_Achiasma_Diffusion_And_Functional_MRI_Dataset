...
oc_mask = nib.load(sys.argv[4])
subj = sys.argv[5]

# Temp
#img = nib.load('/home/rjp/1_OVGU/3_Data_publication/6_SNR_in_chiasm/dwi.nii.gz')
#bvals, bvecs = read_bvals_bvecs('/home/rjp/1_OVGU/3_Data_publication/6_SNR_in_chiasm/dwi.bvals', '/home/rjp/1_OVGU/3_Data_publication/6_SNR_in_chiasm/dwi.bvecs')
#oc_mask = nib.load('/home/rjp/1_OVGU/3_Data_publication/6_SNR_in_chiasm/CON_1_wm_in_chiasm.nii.gz')
# End of temp

# Create gtab
gtab = gradient_table(bvals, bvecs)

# Extract data and affine + resample mask to T1w image
data = img.get_fdata()
affine = img.affine
...
ocmask_good = resample_img(oc_mask, affine, img.shape[0:3], interpolation='continuous')
...
#red = cfa[..., 0]
red = ocmask_data[...]
plt.imshow(np.rot90(data[region,...,0]))
plt.imshow(np.rot90(red[region, ...]), alpha=0.5)
...
#red = cfa[..., 0]
red = ocmask_data[...]
plt.imshow(np.rot90(data[:,:,region2,0]))
#plt.imshow(np.rot90(red[:,:,region2]), alpha=0.25)


plt.subplot(1, 2, 2)
plt.title("CC mask used for SNR computation")
plt.axis('off')
plt.imshow(np.rot90(ocmask_data[...,region2]))
...
ocmask_data_bool = ocmask_data.astype(bool)

mean_signal = np.mean(data[ocmask_data_bool], axis=0)
...
#os.mkdir("output")
with open("output/snr_"+subj+".json", "w") as out_file:
...
with open("output/product_"+subj+".json", "w") as out_file:
